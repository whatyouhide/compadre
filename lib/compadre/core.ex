defmodule Compadre.Core.Success do
  @typedoc """
  Successful result of a parser.

  This is what a successful parsing must return.
  """
  @type t(type) :: %__MODULE__{result: type,
                               bytes_consumed: non_neg_integer,
                               rest: binary}

  defstruct [:result, :bytes_consumed, :rest]
end

defmodule Compadre.Core.Failure do
  @typedoc """
  Failing result of a parser.

  This is what a failed parsing must return.
  """
  @type t(type) :: %__MODULE__{reason: type,
                               bytes_consumed: non_neg_integer,
                               rest: binary}

  defstruct [:reason, :bytes_consumed, :rest]
end

defmodule Compadre.Core.Partial do
...
defmodule Compadre.Core do
  alias Compadre.Core.{Success, Failure, Partial}

  @typedoc """
  The result of a parser.

  This is what each parsing must return. It can be a successful result, a
  failing result, or a partial result. A partial result is just a continuation
  that will keep parsing when fed with more input.
  """
  @type result(succ, fail) ::
    Success.t(succ)
    | Failure.t(fail)
    | Partial.t(result(succ, fail))
